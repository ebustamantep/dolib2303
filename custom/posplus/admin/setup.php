...
/**
 * \file    posplus/admin/setup.php
 * \ingroup posplus
 * \brief   POSPlus setup page.
 */
...
require_once '../lib/posplus.lib.php';
...
// Translations
$langs->loadLangs(array("admin", "posplus@posplus"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('posplussetup', 'globalsetup'));
...
//$formSetup->newItem('POSPLUS_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('POSPLUS_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('POSPLUS_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('POSPLUS_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('POSPLUS_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('POSPLUS_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf POSPLUS_MYPARAM10
$item = $formSetup->newItem('POSPLUS_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('POSPLUS_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('POSPLUS_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('POSPLUS_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/posplus/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=posplus-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'POSPLUS_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'POSPLUS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-posplus page-admin');
...
$head = posplusAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "posplus@posplus");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("POSPlusSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'POSPLUS_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'POSPLUS_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
